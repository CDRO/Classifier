...
function Start-Container {
    ...
    docker run -d --name (Split-Path $PWD -Leaf).ToLower() -p 3000:3000 -v "$(Get-Location)/log:/var/log/$((Split-Path $PWD -Leaf).ToLower())/" -v "$(Get-Location)/app/data:/app/data/" -v "$(Get-Location)/data/Originals_RAW:/data/source" -v "$(Get-Location)/data/Destination:/data/destination" --env-file docker.env (Split-Path $PWD -Leaf).ToLower() | Out-Null
    if ($LASTEXITCODE -ne 0) {
